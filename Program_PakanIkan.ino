...
//Buzzer
#define BUZZER 14                              //pin D5

//Koneksi WiFi & Bot Telegram
String ssid  = "valdodol";                                        //Username
String pass  = "asdfghjk";                                        //Kata sandi
String token = "6164292796:AAFGFpEyhU1Sn5OEqOzm4AnWLWKkW59_APU";  //Token bot telegram
const int id = 1272289088;                                        //ID telegram
...
//Atur Waktu (NOTE : setelah sekali atur langsung disable)
  ...
  //rtc.adjust(DateTime(2023, 4, 14, 12, 39, 0));                                 //Atur manual
}//setup
...
if(myBot.getNewMessage(pesan) ) {                           //Uji jika berhasil menerima pesan dari telegram
  ...
  arrData[2] = "";
  arrData[3] = "";
  for(int i=0; i<message.length(); i++) {                  //Perulangan
  ...
  if(index == 3) {                                         //Parameter lengkap 
    ...
    Serial.println("Jam Makan Diperbarui");                //Setting Jam Makan
    myBot.sendMessage(id, "Pembaruan Berhasil");
    lcd.setCursor(0,0);                                    //Posisi 0,1
    lcd.print("   Pembaruan    ");                         //Menampilkan
    lcd.setCursor(0,1);                                    //Posisi 0,1
...
//Kondisi
  ...
  if(jam == jam1 & menit == menit1 & b!=1){
    kasih_pakan(50);                                        //panggil void kasih pakan
